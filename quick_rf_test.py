#!/usr/bin/env python3
"""
Quick Random Forest Test on Full EEG Pain Classification Dataset
================================================================

Tests Random Forest performance on the complete 51-participant dataset
...
def run_lopocv_evaluation(X, y, groups):
    ...
    logger.info("Starting LOPOCV evaluation...")
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"LOPOCV RESULTS SUMMARY")
    logger.info(f"{'='*60}")
...
def main():
    ...
    logger.info("Starting Quick Random Forest Test on EEG Pain Classification Data")
    ...
    try:
        ...
        # Save results
        output_file = "data/processed/quick_rf_test_results.pkl"
        ...
        logger.info(f"{'='*60}")
        logger.info(f"Random Forest LOPOCV Accuracy: {results['mean_accuracy']:.1f}% ± {results['std_accuracy']:.1f}%")
